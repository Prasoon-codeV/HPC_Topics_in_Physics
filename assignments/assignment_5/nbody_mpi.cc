...
std::tuple<Vec, Vec> initial_conditions() {
    ...
    // Only initialize local portion
    std::mt19937 local_gen(1234 + rank); // thread-local RNG
    std::uniform_real_distribution<> local_ran(0., 1.);

    for (int i = ND_beg; i < ND_end; ++i) {
        x[i] = local_ran(local_gen) * dx + x_min; // Random initial positions
        v[i] = local_ran(local_gen) * dv + v_min; // Random initial velocities
    }

    // Gather the full vectors across all ranks
    if (n_ranks > 1) {
...
std::tuple<Vec, Vec> timestep(const Vec& x0, const Vec& v0, const Vec& m) {
    ...
    Vec x1 = x0, v1 = v0; // Allocate memory
    ...
    // Gather updated positions and velocities
    if (n_ranks > 1) {
...
int main(int argc, char** argv) {
    ...
    if (rank == 0) {
        ...
        std::cout << "Runtime = " << elapsed << " s for N = " << N << std::endl << std::endl;
    }
